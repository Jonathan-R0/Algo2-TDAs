...
buscar_nodo_t* buscar_clave(n_abb_t* arbol, const char* clave,
                            abb_comparar_clave_t comparar_clave,
                            n_abb_t* padre) {
...
bool abb_guardar(abb_t* arbol, const char* clave, void* dato) {
  ...
  if (!arbol->raiz) {  // SI NO HAY RAIZ PONGO LA CLAVE COMO RAIZ
    ...
    buscar_nodo_t* buscado =
        buscar_clave(arbol->raiz, clave, arbol->comparar_clave, arbol->raiz);
    if (!buscado) {
      free(copia_clave);
      return false;
    }
    if (!buscado->hijo) {
      ...
      if (arbol->destruir_dato) arbol->destruir_dato(buscado->hijo->dato);
...
// Borra la clave correspondiente (si esta en la estructura) y devuelve el dato
// o NULL
// Pre: El abb fue creado
...
void* abb_borrar(abb_t* arbol, const char* clave) {
  ...
  if (arbol->comparar_clave(buscado->hijo->clave, buscado->padre->clave) >
      0) {  // ME FIJO SI ES EL HIJO DERECHO O EL IZQUIERDO DEL PADRE
    hijo_derecho = true;
  }

  if (!buscado->hijo->derecha && !buscado->hijo->izquierda) {
    // SI ES UNA HOJA LO ELIMINO Y ACTUALIZO AL PADRE
    if (buscado->hijo == arbol->raiz) arbol->raiz = NULL;
    if (hijo_derecho) {
    ...
    free(buscado->hijo);
  } else if ((!buscado->hijo->derecha && buscado->hijo->izquierda) ||
             (buscado->hijo->derecha && !buscado->hijo->izquierda)) {
    // SI TIENE UN HIJO HIJO DEL BUSCADO
    n_abb_t* nieto;
    ...
    // SI TIENE 2 HIJOS BUSCO EL REEMPLAZANTE,
    // CAMBIO LOS DATOS POR LO DEL REEMPLANZATE
    // BORRO AL REEMPLAZANTE Y ACTUALIZO LOS PADRES
    n_abb_t* reemplazante = buscar_reemplazante(buscado->hijo->derecha);
    n_abb_t* hijo_reemplazante = reemplazante->derecha;
    buscar_nodo_t* buscar_reemplazante = buscar_clave(
        arbol->raiz, reemplazante->clave, arbol->comparar_clave, arbol->raiz);
...
static void _abb_in_order(n_abb_t* arbol,
                          bool visitar(const char* clave, void* dato,
                                       void* extra),
                          void* extra) {
  if (!arbol) return;
