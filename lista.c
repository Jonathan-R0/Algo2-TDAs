...
static nodo_t *nodo_crear2(void *dato) {
...
// Pre: debe recibir un iterador, de largo no nulo, que no sea NULL y cuyo
// actual no se encuentre en NULL.
void *lista_iter_ver_actual(const lista_iter_t *iter) {
  return (iter == NULL || lista_iter_al_final(iter)) ? NULL
                                                     : (iter->actual->valor);
}

// Devuelve true si está el actual en el final de la lista.
// Pre: recibe un iterador diferente de NULL.
// Post: devuelve true si está el actual en el final de la lista y si el
// anterior es el último valor de la lista sobre el cual existe el iterador.
bool lista_iter_al_final(const lista_iter_t *iter) {
  return (iter == NULL)
             ? false
             : (iter->actual == NULL || iter->anterior == iter->lista->ult);
}
...
void *lista_ver_primero(const lista_t *lista) {
  return (lista_esta_vacia(lista)) ? NULL : (lista->prim->valor);
}
...
void *lista_ver_ultimo(const lista_t *lista) {
  return (lista_esta_vacia(lista)) ? NULL : (lista->ult->valor);
}
...
// contrario, se le aplicará la función a cada nodo de la lista y no se
// devolverá nada.
