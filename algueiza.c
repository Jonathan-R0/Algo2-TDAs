...
bool is_digit(char* op, size_t len){
    ...
    for (int j=0;j<len;j++){ if (!isdigit(op[j])) return false; }
    return true;
}
...
char* generar_fecha(char* day){
    ...
    for (int i = 0;i < 6;i++){
        if (!is_digit(date[i],strlen(date[i]))){
            free_strv(fecha); free_strv(date);
            return NULL;
        }
    }
 ...
 * Post: devuelve 1 si el primer vuelo es mayor al segundo, -1 en caso contrario,
 * 0 si son iguales y -2 en caso de error, es decir, si alguno es NULL.
 */
int cmp_abb(const char* info1,const char* info2){
    ...
    int valor = 0;
    for (int i = 5;i > -1;i--){
        if (atoi(fechas1[i]) > atoi(fechas2[i])) valor = 1;
        else if (atoi(fechas1[i]) < atoi(fechas2[i])) valor = -1;
    }
    free_strv(fechas1); free_strv(fechas2);
    if (valor == 0) valor = strcmp(vuelo1[2],vuelo2[2]);
    free_strv(vuelo1); free_strv(vuelo2);
    return valor;
}
...
void imprimir_tablero(char* desde, char* hasta, char** tablero_asc,size_t i){
    ...
    if ((cmp_abb(a_comparar,n_desde) >= 0 && cmp_abb(a_comparar,n_hasta) <= 0) || (cmp_abb2(a_comparar,n_hasta) == 0)) printf("%s - %s\n",info[6],info[0]);
    free(a_comparar);
    free(n_desde);
    free(n_hasta);
    free_strv(info);
...
char** obtener_rangos(abb_t* abb,char* desde,char* hasta){
    ...
    if (vuelos_en_rango == NULL){
        abb_iter_in_destruir(iter); free(n_desde);
        return false;
    }
    char* n_hasta = generar_fecha(hasta);
    if (cmp_abb(abb_iter_in_ver_actual(iter),n_desde) >= 0 && cmp_abb(abb_iter_in_ver_actual(iter),n_hasta) <= 0){
        vuelos_en_rango[i++] = abb_iter_in_ver_dato_actual(iter);
    }
    while (abb_iter_in_avanzar(iter) && cmp_abb(abb_iter_in_ver_actual(iter),n_hasta) <= 0){
...
bool read_file(const char* file,hash_t* hash, abb_t* abb){
    ...
    while((leidos = getline(&linea,&capacidad,archivo)) != -1){
        ...
        if (linea == NULL || !cola_encolar(cola,strdup(linea))){
            cola_destruir(cola,NULL); free(linea); fclose(archivo);
            return false;
        }
    }
    free(linea);
    ...
    while (!cola_esta_vacia(cola)){
        ...
        if (clave_abb == NULL || clave_hash == NULL){
            free(clave_abb); free(clave_hash);
            abb_destruir(abb); hash_destruir(hash); free(vuelo1);
            break;
        }
...
bool info_vuelo(hash_t* hash, char* codigo){
    ...
    free_strv(buscado2); if (buscado3 == NULL) return false;
...
bool prioridad(abb_t* abb,size_t k){
    ...
    if (minimos == NULL) return false;
    if (abb_cantidad(abb) < k) return false;
    abb_iter_t* iter = abb_iter_in_crear(abb);
    if (iter == NULL){ heap_destruir(minimos,NULL); return false; }
    ...
    while (!abb_iter_in_al_final(iter)){
        if (cmp_prioridad(abb_iter_in_ver_dato_actual(iter),heap_ver_tope(minimos)) < 0){
...
bool ver_tablero(abb_t* abb, size_t k, char* type, char* desde, char* hasta){
    if ((strcmp(type,ASCENDENTE) != 0 && strcmp(type,DESCENDENTE) != 0)) return false;
...
void programa(){
    ...
    while (start || (hash_cantidad(hash) > 0 && abb_cantidad(abb) > 0)){
        ...
        if (strcmp(entrada[0],AGREGAR_ARCHIVO) == 0){
            if (entrada[2] != NULL || !read_file(entrada[1],hash,abb)) error = AGREGAR_ARCHIVO;
            else start = false;

        } else if (strcmp(entrada[0],VER_TABLERO) == 0){
            if (!is_digit(entrada[1],strlen(entrada[1])) || entrada[5] != NULL || !ver_tablero(abb,atoi(entrada[1]),entrada[2],entrada[3],entrada[4])) error = VER_TABLERO;

        } else if (strcmp(entrada[0],INFO_VUELO) == 0){
            if (strv_cantidad(entrada,0) != 2 || entrada[2] != NULL || !is_digit(entrada[1],strlen(entrada[1])) || !info_vuelo(hash,entrada[1])) error = INFO_VUELO;

        } else if (strcmp(entrada[0],PRIORIDAD_VUELOS) == 0){
            if (is_digit(entrada[1],strlen(entrada[1])) && atoi(entrada[1]) > 0){
                if (entrada[2] != NULL || !prioridad(abb,(size_t)atoi(entrada[1]))) error = PRIORIDAD_VUELOS;
            } else error = PRIORIDAD_VUELOS;

        } else if (strcmp(entrada[0],BORRAR) == 0){
            if (strv_cantidad(entrada,0) != 3 || entrada[3] != NULL || !borrar(abb,hash,entrada[1],entrada[2])) error = BORRAR;
        }
