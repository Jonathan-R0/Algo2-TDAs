...
static nodo_t* nodo_crear1(void* dato) {
...
// Devuelve verdadero si la cola no tiene elementos encolados, false en caso
// contrario. Pre: la cola fue creada.
bool cola_esta_vacia(const cola_t* cola) { return (!cola->prim && !cola->ult); }
...
void* cola_ver_primero(const cola_t* cola) {
  return (cola->prim == NULL) ? NULL : (cola->prim->valor);
}
