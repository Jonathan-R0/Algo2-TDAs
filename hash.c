...
hash_campo_t* buscar_campo_con_clave(lista_t* lista, const char* clave) {
  ...
  while (!lista_iter_al_final(
      iter)) {  // RECORREMOS LA LISTA CORRESPONDIENTE HASTA ENCONTRAR EL CAMPO
...
bool hash_insertar(hash_t* hash, hash_campo_t* a_guardar, lista_t** listas,
                   size_t capacidad) {
  size_t pos = hashing(a_guardar->clave, strlen(a_guardar->clave)) % capacidad;
  if (!listas[pos]) {  // SI NO TENGO UNA LISTA EN LA POSICION CORRESPONDIENTE
                       // DEL HASH
...
bool redimensionar2(hash_t* hash, bool accion) {
  ...
  nueva_capacidad = (accion) ? hash->capacidad << 1 : hash->capacidad >> 1;
  if (!nuevo) return false;
  nuevo = malloc(sizeof(lista_t*) * nueva_capacidad);
...
void buscar_no_vacia(hash_iter_t* iter) {
  for (size_t i = iter->pos + 1; i < iter->hash->capacidad;
       i++) {  // BUSCO LA SIGUIENTE LISTA NO VACIA
...
hash_t* hash_crear(hash_destruir_dato_t destruir_dato) {
  ...
  for (int i = 0; i < CAPACIDAD_INICIAL_HASH; i++) {
    hash->listas[i] = NULL;
  }  // HAY QUE INICIALIZAR TODOS LOS VALORES CON NULL PARA QUE NO EXPLOTE SI
     // QUEREMOS DESTRUIR UN HASH CON POSICIONES NO INICIALIZADAS
  return hash;
}
...
bool hash_pertenece(const hash_t* hash, const char* clave) {
  if (!hash || hash->cantidad == 0) return false;
  size_t pos_h = hashing(clave, strlen(clave)) % hash->capacidad;
  if (!hash->listas[pos_h] || lista_esta_vacia((hash->listas)[pos_h]))
    return false;
...
bool hash_iter_avanzar(hash_iter_t* iter) {
  ...
  if (!lista_iter_al_final(iter->iter_lista)) {
    ...
    if (lista_iter_al_final(iter->iter_lista)) buscar_no_vacia(iter);
  }
  return true;
}
...
bool hash_iter_al_final(const hash_iter_t* iter) {
  return (iter->hash->cantidad == 0 || iter->pos == iter->hash->capacidad);
}
